...
impl QuoteStringFormat {
    fn to_quote_json(&self) -> Result<Value> {
        let signature = general_purpose::STANDARD.decode(&self.signature)
            .context("Failed to decode signature from base64")?;
        let message = general_purpose::STANDARD.decode(&self.message)
            .context("Failed to decode message from base64")?;
        
        let mut pcrs = Vec::new();
        for pcr_str in &self.pcrs {
            if pcr_str.len() != 64 {
                bail!("PCR should be 64 hex characters (32 bytes), got {}", pcr_str.len());
            }
...
impl TpmVerifier {
    pub fn new(config: config::TpmVerifierConfig) -> Result<Self> {
        ...
        match config.trusted_ak_keys_dir {
            Some(keys_dir) => {
...
impl Verifier for TpmVerifier {
    async fn evaluate(
        ...
        let ev = if let std::result::Result::Ok(guest_ev) = serde_json::from_value::<GuestComponentsEvidence>(evidence.clone()) {
            let quote_json = guest_ev.tpm_quote.to_quote_json()

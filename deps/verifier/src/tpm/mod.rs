...
impl TpmVerifier {
    ...
    pub fn new(config: config::TpmVerifierConfig) -> Result<Self> {
        ...
        let dir_entries = match fs::read_dir(&keys_dir) {
            Ok(entries) => entries,
            Err(e) => {
                log::warn!("Failed to read trusted AK keys directory {:?}: {}", keys_dir, e);
                return Ok(Self { trusted_ak_hashes });
            }
        };
...
mod tests {
    ...
    #[test]
    fn test_tpm_verifier_no_trusted_keys_dir() {
        // When trusted_ak_keys_dir is explicitly set to None, verifier should have no trusted keys
        ...
        assert!(
            verifier.trusted_ak_hashes.is_empty(),
            "Verifier with trusted_ak_keys_dir=None should have no trusted AK hashes"
        );
    }
    ...
    fn test_tpm_verifier_default_config() {
        ...
        assert!(
            config.trusted_ak_keys_dir.is_some(),
            "Default config should have a trusted_ak_keys_dir set"
        );
        assert_eq!(
            config.trusted_ak_keys_dir.unwrap().to_str().unwrap(),
            DEFAULT_TRUSTED_AK_KEYS_DIR
        );
    }

    #[test]
    fn test_tpm_verifier_default_nonexistent_dir() {
        // If the default directory doesn't exist, verifier should still initialize
        // but with no trusted keys loaded
        let config = config::TpmVerifierConfig::default();
        // This will try to read /etc/tpm/ak_pub_keys which likely doesn't exist in test env
        let verifier = TpmVerifier::new(config).unwrap();
        // The verifier should initialize successfully even if the directory doesn't exist
